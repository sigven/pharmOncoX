...
get_unambiguous_gene_aliases <- function(
  ...
  gene_info_fname <- file.path(basedir, "data-raw", "gene_info","gene_info.rds")
  ...
  gene_dictionary <- gene_dictionary %>%
    dplyr::bind_rows(
      data.frame(symbol='CDKN2A',alias='p16ink4a',entrezgene=1029, stringsAsFactors = F)) %>%
    dplyr::bind_rows(
      data.frame(symbol='CDKN1A',alias='p21',entrezgene=1026, stringsAsFactors = F)) %>%
    dplyr::bind_rows(
      data.frame(symbol='ERBB2',alias='HER-2/Neu',entrezgene=2064, stringsAsFactors = F)) %>%
    dplyr::bind_rows(
      data.frame(symbol='ERBB2',alias='HER2/Neu',entrezgene=2064, stringsAsFactors = F)) %>%
    dplyr::bind_rows(
      data.frame(symbol='TP53',alias='p53',entrezgene=7157, stringsAsFactors = F)) %>%
    dplyr::bind_rows(
      data.frame(symbol='ABL1',alias='BCR-ABL1',entrezgene=25, stringsAsFactors = F)) %>%
    dplyr::bind_rows(
      data.frame(symbol='PDCD1',alias='PD-1',entrezgene=5133, stringsAsFactors = F)) %>%
    dplyr::bind_rows(
      data.frame(symbol='BCL2',alias='BCL-2',entrezgene=596, stringsAsFactors = F)) %>%
    dplyr::bind_rows(
      data.frame(symbol='TNFRSF17',alias='BCMA',entrezgene=608, stringsAsFactors = F)) %>%
    dplyr::arrange(symbol)

  return(gene_dictionary)
}

get_genomics_england_panels <-
  function(basedir = NULL,
    ...
    panels <- read.table(file=panel_id_file, header=T, sep="\t",
                         stringsAsFactors = F,quote="")
    i <- 1
    all_panels <- data.frame()
    while(i <= nrow(panels)){

      api_url <- paste0('https://panelapp.genomicsengland.co.uk/api/v1/panels/',panels[i,]$id)
      panel_name <- panels[i,]$name
      contents <- jsonlite::fromJSON(api_url)
      outfile <- paste0(
        basedir,'/cancer_predisposition/genomics_england/panel_',panels[i,]$id,'.tsv')

      gene_phenotypes <- c()
      for(n in 1:length(contents$genes$phenotypes)){
        phenotypes <- paste(
          unique(stringr::str_replace_all(contents$genes$phenotypes[[n]],"\\{|\\}|\\?","")), collapse=";")
        gene_phenotypes <- c(gene_phenotypes, phenotypes)
      }

      if(build == "grch38"){
        df <- data.frame(
          'hgnc_id' = stringr::str_replace(contents$genes$gene_data$hgnc_id,'HGNC:',''),
          'biotype' = contents$genes$gene_data$biotype,
          'ensembl_gene_id' = contents$genes$gene_data$ensembl_genes$GRch38$`90`$ensembl_id,
          'mode_of_inheritance' = contents$genes$mode_of_inheritance,
          'penetrance' = contents$genes$penetrance,
          'confidence_level' = contents$genes$confidence_level,
          'panel_name' = contents$name,
          'panel_id' = contents$id,
          'panel_version' = contents$version,
          'phenotypes' = gene_phenotypes,
          stringsAsFactors = F)
      }else{
        df <- data.frame(
          'hgnc_id' = stringr::str_replace(contents$genes$gene_data$hgnc_id,'HGNC:',''),
          'biotype' = contents$genes$gene_data$biotype,
          'ensembl_gene_id' = contents$genes$gene_data$ensembl_genes$GRch37$`82`$ensembl_id,
          'mode_of_inheritance' = contents$genes$mode_of_inheritance,
          'penetrance' = contents$genes$penetrance,
          'confidence_level' = contents$genes$confidence_level,
          'panel_name' = contents$name,
      ...
      df$id <- i
      df$panel_url <- paste0('https://panelapp.genomicsengland.co.uk/panels/',df$panel_id)
    ...
    all_panels <- all_panels %>%
      dplyr::mutate(
        predisp_cancer_moi_ge = dplyr::if_else(
          stringr::str_detect(mode_of_inheritance,"MONOALLELIC, autosomal or pseudoautosomal, NOT imprinted") |
            stringr::str_detect(mode_of_inheritance,"MONOALLELIC, autosomal or pseudoautosomal, imprinted status unknown"),
          "AD",as.character(NA))) %>%
      dplyr::mutate(predisp_cancer_moi_ge = dplyr::if_else(
        stringr::str_detect(mode_of_inheritance,"BIALLELIC, autosomal or pseudoautosomal"),
        "AR",as.character(predisp_cancer_moi_ge))) %>%
      dplyr::mutate(predisp_cancer_moi_ge = dplyr::if_else(
        stringr::str_detect(mode_of_inheritance,"BOTH monoallelic and biallelic"),
        "AD/AR",as.character(predisp_cancer_moi_ge))) %>%
      dplyr::select(-mode_of_inheritance) %>%
      dplyr::left_join(
        dplyr::select(gene_info, hgnc_id, symbol,
                      entrezgene, name),by="hgnc_id") %>%
      #dplyr::left_join(dplyr::select(gene_info, hgnc_id, symbol, entrezgene, name),by="hgnc_id") %>%
    ...
    panel_df <- as.data.frame(
      all_panels %>%
        dplyr::mutate(
          tmp =
            paste(paste0("GE_PANEL_",id),
                  confidence_level,panel_id,
                  panel_version,predisp_cancer_moi_ge,
                  sep=":")) %>%
        #dplyr::group_by(symbol) %>%
        dplyr::group_by(entrezgene, symbol) %>%
        dplyr::summarise(
          ge_panel = paste(tmp, collapse="&"),
          moi = paste(unique(sort(predisp_cancer_moi_ge)), collapse=","),
          phenotypes = paste(unique(phenotypes), collapse=", "),
          .groups = "drop") %>%
        #dplyr::select(symbol, phenotypes, moi, ge_panel) %>%
        dplyr::select(symbol, entrezgene, phenotypes, moi) %>%
        dplyr::mutate(phenotypes = stringr::str_replace_all(
          phenotypes,"^(\\s{0,},\\s{1,})","")) %>%
        dplyr::mutate(source_db = "PANELAPP")

    )

    return(panel_df)
    # ret <- list('ge_panel_df' = panel_df, 'ge_panel_df_all' = all_panels)
    #
    # return(ret)
  }
...
get_tcga_germline_genes <- function(
  basedir = NULL,
  gene_info = NULL){
  ...
  tcga_germline_genes <-
    openxlsx::read.xlsx(
      file.path(basedir, "data-raw",
                "biomarkers", "other",
                "ding_pancancer_cell_2018.xlsx"),
      sheet = 1) %>%
    janitor::clean_names() %>%
    dplyr::select(gene_symbol, mode_of_inheritance,
    ...
    dplyr::mutate(moi = dplyr::if_else(
      stringr::str_detect(
        mode_of_inheritance,"autosomal dominant"),"AD",as.character(NA))) %>%
    dplyr::mutate(moi = dplyr::if_else(
      stringr::str_detect(
        mode_of_inheritance,"autosomal recessive"),"AR",as.character(moi))) %>%
    dplyr::mutate(moi = dplyr::if_else(
      stringr::str_detect(
    ...
    dplyr::mutate(phenotypes = paste(tumor_type,cancer_syndrome,sep="; ")) %>%
    dplyr::select(-c(cancer_syndrome, mode_of_inheritance, tumor_type)) %>%
    dplyr::left_join(
      dplyr::select(gene_info, symbol, entrezgene), by = c("symbol")) %>%
    dplyr::mutate(source_db = "TCGA_PANCAN_2018")

  return(tcga_germline_genes)
...
get_intogen_driver_genes <- function(
  ...
  intogen_drivers <- as.data.frame(
    read.table(file = file.path(
      basedir, "data-raw","biomarkers",
      "intogen", "Compendium_Cancer_Genes.tsv"),
      sep="\t",
      ...
      dplyr::summarise(intogen_driver = paste(unique(Hmisc::capitalize(role)),collapse="&"),
                       intogen_ttypes = paste(unique(cancer_type),collapse="&"),
                       .groups = "drop") %>%
      dplyr::mutate(source_db = "INTOGEN_20200201") %>%
      dplyr::left_join(dplyr::select(gene_info,
                                     symbol, entrezgene),
                       by = "symbol") %>%
      dplyr::rename(moi = intogen_driver,
                    phenotypes = intogen_ttypes)
  )
...
get_cancer_gene_census <- function(basedir = NULL,
                                   origin = "germline",
                                   cgc_version = "94"){

  cosmic_cgc <- read.table(
    file.path(basedir,
              "data-raw",
              "biomarkers",
              "cosmic",
    ...
    na.strings="", comment.char = "#", fill=T) %>%
    janitor::clean_names()
  ...
  cosmic_cgc <- cosmic_cgc %>%
    dplyr::select(gene_symbol, entrez_gene_id, hallmark, germline, somatic, tier, molecular_genetics, tumour_types_germline) %>%
    dplyr::rename(symbol = gene_symbol, entrezgene = entrez_gene_id, moi = molecular_genetics, phenotypes = tumour_types_germline) %>%
    ...
    dplyr::mutate(moi = dplyr::if_else(stringr::str_detect(moi,"Rec/X"),as.character(NA),as.character(moi))) %>%
    dplyr::select(symbol, moi, entrezgene, phenotypes) %>%
    ## bug in cancer gene census for ERCC5
    dplyr::mutate(entrezgene = dplyr::if_else(symbol == 'ERCC5',as.integer(2073),as.integer(entrezgene))) %>%
    dplyr::mutate(source_db = "CGC94")

  rlogging::message(paste0("Found n = ", nrow(cosmic_cgc),
...
#   file=gzfile(paste0("/Users/sigven/research/DB/var_annotation_tracks/clinvar/",
#                      "clinvar_", cancer_biomarker[['version']][['ClinVar']],
#                      "_grch38.tsv.gz")),
#               header = T, quote = "", sep = "\t",
#               na.strings = c(".","NA"), comment.char = "") %>%
#   dplyr::filter(classification == "Pathogenic" |
